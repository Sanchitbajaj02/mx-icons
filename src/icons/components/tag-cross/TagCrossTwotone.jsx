import React from "react";
import Icon from "../../Icon";

export default function TagCrossTwotone({
  size = 24,
  color = "#292D32",
  className = "",
  ...props
}) {
  return (
    <Icon
      size={size}
      color={color}
      fill="none"
      className={className}
      {...props}
    >
      <path d="M10.2797 20.25H16.9997C19.7597 20.25 21.9997 18.01 21.9997 15.25V8.75C21.9997 5.99 19.7597 3.75 16.9997 3.75H10.2797C8.86969 3.75 7.52969 4.34 6.57969 5.39L3.04969 9.27C1.63969 10.82 1.63969 13.18 3.04969 14.73L6.57969 18.61C7.52969 19.66 8.86969 20.25 10.2797 20.25Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
<g opacity="0.4">
<path d="M16.0005 14.4703L11.0605 9.53027" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
<path d="M11.0605 14.4703L16.0005 9.53027" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
</g>
    </Icon>
  );
}
